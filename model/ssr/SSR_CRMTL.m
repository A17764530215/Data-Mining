...
%         [ Alpha1 ] = Solver(H1, -e, lb, ub, opts.solver);
    end
...
%             [ Alpha1(R) ] = Solver(H1(R,R), f, lb, ub, opts.solver);
        end
    end

    function [ x ] = Solver(H, f, lb, ub, opts)
        func = @(x) 0.5*x*H*x'+x*f;
        x0 = zeros(size(lb));
        x = fmincon(func, x0', [], [], [], [], lb, ub, [], opts);
        x = x';
    end
