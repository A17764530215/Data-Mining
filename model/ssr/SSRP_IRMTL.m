function [  CVStat, CVTime, CVRate ] = SSRP_IRMTL( xTrain, yTrain, xTest, yTest, TaskNum, IParams, opts )
...
for i = 1 : n
    ...
    if i == 1 ||  ~EqualsTo(Params, LastParams)
        ...
        [ Alpha{i} ] = SSR_P(H1, H2, Alpha{i-1}, Params.C);
...
%% Compare
    function [ b ] = EqualsTo(p1, p2)
        ...
        if strcmp(k1.kernel, 'rbf') && strcmp(k2.kernel, 'rbf')
            b = p1.C == p1.C && p1.mu == p2.mu;
        else
            throw(MException('SSRP_IRMTL', 'EqualsTo'));
        end
    end
...
%% SSR for $p$
    function [ Alpha2 ] = SSR_P(H1, H2, Alpha1, Params)
        % safe screening rules for $p$
        P = chol(H2, 'upper');
        LL = (H1+H2)*Alpha1;
        RL = sqrt(sum(P.*P, 1))';
